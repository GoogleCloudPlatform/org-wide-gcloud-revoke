...
function debugApiAccess() {
  ...
  try {
    ...
    if (typeof AdminDirectory === 'undefined') {
      Logger.log('❌ AdminDirectory service not found. Please enable Admin SDK API in Services.');
      return;
    }
...
function listUsersOnly() {
  ...
  try {
    ...
    users.forEach((user, index) => {
      Logger.log('%s. %s (%s)', index + 1, user.primaryEmail, user.name?.fullName || 'No name');
    });
 ...
 */
function listUsersAndInvalidate(dryRun = false, maxUsers = MAX_USERS_TO_PROCESS) {
  const startTime = new Date();
  
  // Log execution details
  Logger.log('=== gcloud Token Revocation Script ===');
  Logger.log('Start time: %s', startTime.toISOString());
  Logger.log('Mode: %s', dryRun ? 'DRY RUN (no actual revocation)' : 'LIVE (will revoke tokens)');
  ...
  try {
    ...
    for (let i = 0; i < users.length && i < maxUsers; i++) {
      ...
      Logger.log('Processing user %s/%s: %s', i + 1, Math.min(users.length, maxUsers), user.primaryEmail);
      ...
      try {
        ...
        stats.errors++;
        Logger.log('  ❌ Error processing user %s: %s', user.primaryEmail, userError.toString());
      }
